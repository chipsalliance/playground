...
class XDMABlackBox(c: XDMAParams) extends BlackBox {
  ...
  val bars = c.bars.zip(c.basesFull).zipWithIndex.map {
    case ((a, b), i) =>
...
/** a fork version to [[sifive.fpgashells.ip.xilinx.xdma]]. */
  ...
  val device = new SimpleDevice("pci", Seq("xlnx,xdma-host-3.00")) {
    override def describe(resources: ResourceBindings): Description = {
      ...
      val extra = Map(
        ...
        "interrupt-controller" -> Seq(
          ResourceMap(
            labels = Seq(intc),
            value = Map("interrupt-controller" -> Nil, "#address-cells" -> ofInt(0), "#interrupt-cells" -> ofInt(1))
          )
        )
      )
  ...
  val control = AXI4SlaveNode(
    Seq(
      AXI4SlavePortParameters(
        ...
        ), // AXI4-Lite never interleaves responses
        beatBytes = 4
      )
    )
  )
  ...
  val master = AXI4MasterNode(
    Seq(
      AXI4MasterPortParameters(
        masters = Seq(AXI4MasterParameters(name = c.name, id = IdRange(0, 1 << c.mIDBits), aligned = false))
      )
    )
  )
